...
const ParentPortal: React.FC<ParentPortalProps> = ({ 
  ...
  const handleDuplicateChore = (chore: Chore) => {
      onAddChore({ ...chore, title: `${chore.title} (Copy)`, completedBy: [] });
      setActiveMenuChoreId(null);
  };
  ...
  const handleSaveChore = () => {
      ...
      const assignments = Array.from(selectedKids).map(kidId => ({ userId: kidId, points: kidPoints[kidId] || 0 }));
      const choreData = { title: choreTitle, description: choreDesc, assignments, frequency, frequencyConfig: freqConfig, timeOfDay, completedBy: [], dueDate: new Date().toISOString(), icon: selectedIcon };
      const existingChore = chores.find(c => c.id === editingChoreId);
      if (editingChoreId && existingChore) { onUpdateChore({ ...existingChore, ...choreData }); } else { onAddChore(choreData); }
      resetForm();
  };
  ...
  const handleSaveReward = () => {
      ...
      const rewardData = { title: rewardTitle, cost: rewardCost, isShared: isSharedReward, image: `https://ui-avatars.com/api/?name=${encodeURIComponent(rewardTitle)}&background=random`, approved: true };
      if (editingRewardId) { const existing = rewards.find(r => r.id === editingRewardId); if (existing) { onUpdateReward({ ...existing, ...rewardData }); } } else { onAddReward({ ...rewardData, id: `r-${Date.now()}` }); }
      setRewardTitle('');
      setRewardCost(100);
  ...
  const handleRefreshCalendars = async () => {
      ...
      setCalError(null);
      try { const cals = await fetchCalendarList(googleAccessToken); setFetchedCalendars(cals); } catch (e: any) { console.error(e); setCalError(e.message || "Failed to load calendars"); }
  ...
  const handleRefreshAlbums = async () => {
      ...
      setPhotoError(null);
      try { const albums = await fetchAlbums(googleAccessToken); setFetchedAlbums(albums); } catch (e: any) { console.error(e); setPhotoError(e.message || "Failed to load albums"); }
  ...
  const getWeeklyPointsData = () => {
     ...
     return dates.map(date => {
         ...
         kids.forEach(kid => {
             const points = choreHistory.filter(log => log.date === date && log.userId === kid.id).reduce((sum, log) => sum + log.points, 0);
      ...
      <div className="flex-1 overflow-y-auto p-6 md:p-10">
        {activeTab === 'overview' && (
           <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                 {kids.map(kid => (<div key={kid.id} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200"><div className="flex items-center gap-4 mb-4"><img src={kid.avatar} className="w-12 h-12 rounded-full" alt={kid.name}/><div><h3 className="font-bold text-lg">{kid.name}</h3><p className="text-sm text-slate-500">Kid Account</p></div></div><div className="flex justify-between items-end"><div><p className="text-xs uppercase tracking-wider text-slate-400 font-bold">Current Points</p><p className="text-3xl font-black text-blue-600">{kid.points}</p></div><div className="text-right"><p className="text-xs uppercase tracking-wider text-slate-400 font-bold">Lifetime</p><p className="text-lg font-bold text-slate-700">{kid.totalPointsEarned}</p></div></div></div>))}
                    ...
                    <div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm h-96">
                        ...
                        <ResponsiveContainer width="100%" height="80%">
                            <BarChart data={getWeeklyPointsData()}>
                                ...
                                {kids.map((kid, index) => (<Bar key={kid.id} dataKey={kid.name} fill={['#3b82f6', '#ec4899', '#8b5cf6', '#10b981'][index % 4]} radius={[4, 4, 0, 0]} barSize={40} />))}
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                </div>
        ...
        {activeTab === 'chores' && (
          <div className="space-y-8">
             ...
             {isCreatingChore && (<div className="bg-white p-8 rounded-2xl shadow-lg border border-slate-200 animate-in fade-in slide-in-from-top-4"><div className="flex justify-between items-center mb-6"><h3 className="text-lg font-bold text-slate-800">{editingChoreId ? 'Edit Chore' : 'Create New Chore'}</h3></div><div className="grid grid-cols-1 md:grid-cols-2 gap-8"><div className="space-y-6"><div><label className="block text-sm font-bold text-slate-700 mb-2">Chore Title</label><input type="text" value={choreTitle} onChange={(e) => setChoreTitle(e.target.value)} placeholder="e.g. Make Bed, Wash Dishes" className="w-full px-4 py-3 rounded-xl border border-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"/></div><div><label className="block text-sm font-bold text-slate-700 mb-2">Select Icon</label><div className="grid grid-cols-5 gap-3">{CHORE_ICONS.map((icon, idx) => (<button key={idx} onClick={() => setSelectedIcon(icon.svg)} className={`p-3 rounded-xl flex items-center justify-center transition-all ${selectedIcon === icon.svg ? 'bg-blue-600 text-white shadow-md scale-105' : 'bg-slate-50 text-slate-400 hover:bg-slate-100 hover:text-slate-600'}`} title={icon.name}><div className="w-6 h-6" dangerouslySetInnerHTML={{ __html: icon.svg }} /></button>))}</div></div><div><label className="block text-sm font-bold text-slate-700 mb-2">Description (Optional)</label><textarea value={choreDesc} onChange={(e) => setChoreDesc(e.target.value)} placeholder="e.g. Put pillows in place and fold blanket" className="w-full px-4 py-3 rounded-xl border border-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none h-20"/></div><div><label className="block text-sm font-bold text-slate-700 mb-2 flex items-center gap-2"><Clock size={16} className="text-slate-400"/> Time of Day</label><div className="grid grid-cols-2 sm:grid-cols-4 gap-2">{(['morning', 'afternoon', 'evening', 'all_day'] as const).map(t => (<button key={t} onClick={() => setTimeOfDay(t)} className={`px-2 py-2 rounded-lg text-sm font-medium border transition-colors capitalize ${timeOfDay === t ? 'bg-blue-100 border-blue-300 text-blue-700' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}>{t.replace('_', ' ')}</button>))}</div></div></div><div className="space-y-6"><div className="bg-slate-50 p-5 rounded-xl border border-slate-200"><label className="block text-sm font-bold text-slate-700 mb-3 flex items-center gap-2"><UserPlus size={16} className="text-slate-400"/> Assign To & Set Points</label><div className="space-y-3">{kids.length === 0 && <p className="text-sm text-red-500">No kids found. Add them in settings.</p>}{kids.map(kid => (<div key={kid.id} className="flex items-center justify-between bg-white p-3 rounded-lg border border-slate-200"><div className="flex items-center gap-3"><input type="checkbox" checked={selectedKids.has(kid.id)} onChange={() => handleKidSelection(kid.id)} className="w-5 h-5 rounded text-blue-600 focus:ring-blue-500 border-gray-300"/><span className="font-medium text-slate-700">{kid.name}</span></div>{selectedKids.has(kid.id) && (<div className="flex items-center gap-2"><input type="number" value={kidPoints[kid.id]} onChange={(e) => handlePointChange(kid.id, parseInt(e.target.value) || 0)} className="w-20 px-2 py-1 border border-slate-300 rounded text-right"/><span className="text-xs text-slate-500 font-bold">PTS</span></div>)}</div>))}</div></div><div className="bg-slate-50 p-5 rounded-xl border border-slate-200"><label className="block text-sm font-bold text-slate-700 mb-3 flex items-center gap-2"><Repeat size={16} className="text-slate-400"/> Frequency</label><div className="flex gap-2">{(['daily', 'weekly', 'monthly'] as const).map(f => (<button key={f} onClick={() => { setFrequency(f); setFreqConfig(f === 'weekly' ? 'Monday' : (f === 'monthly' ? '1' : 'all')); }} className={`flex-1 py-2 rounded-lg text-sm font-medium border transition-colors capitalize ${frequency === f ? 'bg-blue-100 border-blue-300 text-blue-700' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}>{f}</button>))}</div>{renderFreqConfig()}</div></div></div><div className="mt-8 flex justify-end gap-4 border-t border-slate-100 pt-6"><button onClick={resetForm} className="px-6 py-2.5 rounded-xl font-bold text-slate-500 hover:bg-slate-100">Cancel</button><button onClick={handleSaveChore} className="px-8 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl flex items-center gap-2 shadow-lg shadow-blue-200"><Save size={20} />{editingChoreId ? 'Update Chore' : 'Save Chore'}</button></div></div>)}
             <div className="bg-white p-6 rounded-2xl border border-slate-200 min-h-[400px]"><h3 className="text-xl font-bold mb-6 text-slate-800">Active Chores List</h3><div className="overflow-visible"><table className="w-full text-left border-collapse"><thead><tr className="border-b border-slate-100"><th className="pb-3 text-slate-400 font-medium text-sm pl-4 w-16">Icon</th><th className="pb-3 text-slate-400 font-medium text-sm">Chore</th><th className="pb-3 text-slate-400 font-medium text-sm">Frequency</th><th className="pb-3 text-slate-400 font-medium text-sm">Assigned To</th><th className="pb-3 text-slate-400 font-medium text-sm text-right pr-4 w-16">Action</th></tr></thead><tbody className="divide-y divide-slate-50">{chores.map(chore => (<tr key={chore.id} className="group hover:bg-slate-50"><td className="py-4 pl-4 align-top"><div className="w-10 h-10 bg-blue-50 text-blue-600 rounded-lg flex items-center justify-center p-2" dangerouslySetInnerHTML={{ __html: chore.icon }} /></td><td className="py-4 font-medium text-slate-700 align-top">{chore.title}<div className="text-xs text-slate-400 capitalize">{chore.timeOfDay.replace('_', ' ')}</div></td><td className="py-4 text-slate-600 text-sm align-top"><span className="capitalize font-medium">{chore.frequency}</span><span className="text-slate-400 block text-xs">{chore.frequencyConfig === 'all' ? 'Every Day' : chore.frequencyConfig}</span></td><td className="py-4 align-top"><div className="flex flex-col gap-1">{chore.assignments.map(a => { const user = users.find(u => u.id === a.userId); if (!user) return null; return (<div key={a.userId} className="flex items-center gap-2 text-sm"><img src={user.avatar} className="w-5 h-5 rounded-full" alt={user.name}/><span className="text-slate-600">{user.name} <span className="text-slate-400 text-xs">({a.points} pts)</span></span></div>); })}</div></td><td className="py-4 text-right pr-4 align-top relative"><button onClick={() => setActiveMenuChoreId(activeMenuChoreId === chore.id ? null : chore.id)} className={`p-2 rounded-lg transition-colors ${activeMenuChoreId === chore.id ? 'bg-blue-100 text-blue-600' : 'text-slate-300 hover:text-slate-600 hover:bg-slate-100'}`}><MoreVertical size={18} /></button>{activeMenuChoreId === chore.id && (<div className="absolute right-8 top-12 w-48 bg-white rounded-xl shadow-xl border border-slate-100 z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200"><button onClick={() => handleEditChore(chore)} className="w-full text-left px-4 py-3 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-3 border-b border-slate-50"><Edit size={16} className="text-blue-500"/> Edit Task</button><button onClick={() => handleDuplicateChore(chore)} className="w-full text-left px-4 py-3 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-3 border-b border-slate-50"><Copy size={16} className="text-purple-500"/> Duplicate</button><button onClick={() => onDeleteChore(chore.id)} className="w-full text-left px-4 py-3 text-sm text-red-600 hover:bg-red-50 flex items-center gap-3"><Trash2 size={16} /> Delete</button></div>)}</td></tr>))}</tbody></table></div></div>
          </div>
        )}
        {activeTab === 'rewards' && (
           <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in fade-in slide-in-from-bottom-4">
                         <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col"><div className="mb-4 flex items-center justify-between"><div><h4 className="font-bold text-lg text-slate-800 flex items-center gap-2"><CalIcon size={20} className="text-blue-500"/> Calendar Sync</h4><p className="text-sm text-slate-500">Choose calendars to display.</p></div><div className="flex gap-2">{calError && (<button onClick={handleResetPermissions} className="text-orange-500 hover:text-orange-600 bg-orange-50 px-3 py-1.5 rounded-lg text-xs font-bold">Reset Permissions</button>)}<button onClick={handleRefreshCalendars} disabled={refreshingCal} className="text-slate-400 hover:text-blue-600 disabled:opacity-50 transition-colors p-2 rounded-full hover:bg-slate-50" title="Refresh Calendars">{refreshingCal ? <Loader2 size={20} className="animate-spin text-blue-500"/> : <RefreshCw size={20}/>}</button></div></div>
                             <div className="bg-slate-50 p-4 rounded-xl space-y-3 mb-6">
                                 ...
                                 {calError && (<div className="p-3 bg-red-50 text-red-600 rounded-lg text-sm mb-2 flex items-start gap-2"><AlertTriangle size={16} className="mt-0.5 flex-shrink-0"/><div><p className="font-bold">Error loading calendars</p><p className="text-xs opacity-80">{calError}</p></div></div>)}
                                 {fetchedCalendars.length > 0 ? (<div className="grid grid-cols-1 gap-2 max-h-60 overflow-y-auto pr-1">{fetchedCalendars.map(cal => { const isSynced = calendarSources.some(s => s.googleCalendarId === cal.id); return (<div key={cal.id} className="flex items-center justify-between bg-white p-3 rounded-lg border border-slate-200"><span className="font-medium text-slate-700 truncate flex-1 mr-2 text-sm">{cal.summary}</span>{isSynced ? (<button onClick={() => { const source = calendarSources.find(s => s.googleCalendarId === cal.id); if(source) onRemoveCalendarSource(source.id); }} className="text-red-500 text-xs font-bold hover:underline">Remove</button>) : (<button onClick={() => handleSyncCalendar(cal)} className="bg-blue-600 text-white px-3 py-1 rounded-lg text-xs font-bold hover:bg-blue-700 transition-colors">Sync</button>)}</div>); })}</div>) : (<div className="text-center py-4 text-slate-400 text-sm italic">{refreshingCal ? 'Loading calendars...' : (!calError && 'No calendars found.')}</div>)}
                             </div>
                             {calendarSources.length > 0 && (<div className="mt-auto pt-4 border-t border-slate-100"><h5 className="text-xs font-bold text-slate-400 uppercase mb-2">Active Syncs</h5><div className="space-y-2">{calendarSources.map(source => (<div key={source.id} className={`flex items-center justify-between p-2 rounded-lg border text-sm ${source.color}`}><div className="truncate pr-2"><span className="font-bold">{source.name}</span><span className="text-xs opacity-70 ml-2">({source.type === 'personal' ? 'Personal' : 'Family'})</span></div><button onClick={() => onRemoveCalendarSource(source.id)} className="p-1 hover:bg-white/50 rounded"><Trash2 size={14}/></button></div>))}</div></div>)}
                         </div>

                         <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col"><div className="mb-4 flex items-center justify-between"><div><h4 className="font-bold text-lg text-slate-800 flex items-center gap-2"><ImageIcon size={20} className="text-purple-500"/> Photo Frame</h4><p className="text-sm text-slate-500">Select an album for the idle screensaver.</p></div><div className="flex gap-2">{photoError && (<button onClick={handleResetPermissions} className="text-orange-500 hover:text-orange-600 bg-orange-50 px-3 py-1.5 rounded-lg text-xs font-bold">Reset Permissions</button>)}<button onClick={handleRefreshAlbums} disabled={refreshingPhotos} className="text-slate-400 hover:text-purple-600 disabled:opacity-50 transition-colors p-2 rounded-full hover:bg-slate-50" title="Refresh Albums">{refreshingPhotos ? <Loader2 size={20} className="animate-spin text-purple-500"/> : <RefreshCw size={20}/>}</button></div></div>
                             {photoError && (
                             ...
                             {fetchedAlbums.length > 0 ? (<div className="grid grid-cols-1 gap-3 max-h-96 overflow-y-auto p-1">{fetchedAlbums.map(album => (<button key={album.id} onClick={() => handleSyncAlbum(album)} className={`p-3 rounded-xl border text-left transition-all flex items-center gap-3 ${photoConfig.albumId === album.id ? 'bg-purple-50 border-purple-300 ring-2 ring-purple-200' : 'bg-white border-slate-200 hover:border-purple-300 hover:shadow-md'}`}><div className="w-16 h-16 bg-slate-100 rounded-lg flex-shrink-0 overflow-hidden flex items-center justify-center text-slate-400">{album.coverPhotoBaseUrl ? (<img src={`${album.coverPhotoBaseUrl}=w500-h500-c`} className="w-full h-full object-cover" alt=""/>) : <ImageIcon size={24}/>}</div><div className="overflow-hidden flex-1"><h5 className="font-bold text-slate-800 truncate text-sm">{album.title}</h5><p className="text-xs text-slate-500">{album.mediaItemsCount} items</p></div>{photoConfig.albumId === album.id && <div className="ml-auto text-purple-600"><CheckCircle2 size={20}/></div>}</button>))}</div>) : (<div className="text-center py-8 text-slate-400 text-sm italic bg-slate-50 rounded-xl border border-dashed border-slate-200">{refreshingPhotos ? 'Loading albums...' : (!photoError && 'No albums found.')}</div>)}
                             {photoConfig.albumName && (<div className="mt-4 pt-4 border-t border-slate-100"><div className="flex items-center gap-2 text-sm text-purple-700 bg-purple-50 p-3 rounded-xl border border-purple-100"><CheckCircle2 size={16}/><span>Currently showing: <strong>{photoConfig.albumName}</strong></span></div></div>)}
                         </div>
                    </div>
                 )}
            </div>
        ...
        {activeTab === 'settings' && (
          <div className="space-y-8 max-w-4xl">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200"><h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2"><Settings size={20} className="text-slate-400"/> General</h3><div className="flex gap-4 items-end"><div className="flex-1"><label className="block text-sm font-medium text-slate-500 mb-1">Family Name</label><input type="text" value={editingFamilyName} onChange={(e) => setEditingFamilyName(e.target.value)} className="w-full px-4 py-2 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500"/></div><button onClick={() => onUpdateFamilyName(editingFamilyName)} className="px-6 py-2.5 bg-slate-800 text-white font-bold rounded-xl hover:bg-slate-700 flex items-center gap-2"><Save size={18} /> Save</button></div></div>
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200"><h3 className="text-lg font-bold text-slate-800 mb-6 flex items-center gap-2"><UserPlus size={20} className="text-slate-400"/> Family Members</h3><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">{users.map(user => (<div key={user.id} className="border border-slate-200 rounded-xl p-4 flex items-center justify-between"><div className="flex items-center gap-3"><img src={user.avatar} className="w-10 h-10 rounded-full bg-slate-100" /><div><p className="font-bold text-slate-700">{user.name}</p><span className={`text-xs px-2 py-0.5 rounded-full font-bold ${user.role === UserRole.PARENT ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}>{user.role}</span></div></div><button onClick={() => onDeleteUser(user.id)} className="text-slate-300 hover:text-red-500 p-2" title="Remove User"><Trash2 size={16} /></button></div>))}</div><div className="bg-slate-50 p-6 rounded-xl border border-slate-200"><h4 className="font-bold text-slate-700 mb-4">Add New Member</h4><form onSubmit={handleAddUserSubmit} className="flex flex-col md:flex-row gap-4"><input type="text" placeholder="Name (e.g., Mom, Leo)" required value={newName} onChange={(e) => setNewName(e.target.value)} className="flex-1 px-4 py-2 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500"/><select value={newRole} onChange={(e) => setNewRole(e.target.value as UserRole)} className="px-4 py-2 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"><option value={UserRole.KID}>Kid</option><option value={UserRole.PARENT}>Parent</option></select><button type="submit" className="px-6 py-2 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700">Add Member</button></form></div></div>
          </div>
        )}
      </div>
    </div>
